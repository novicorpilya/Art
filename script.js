...
const modals = () => {
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', () => {
      // let scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); для старых браузеров после вместо document.documentElement.scrollHeight 
      // используем переменную scrollHeight
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', function () {
    (0,_services_requests__WEBPACK_IMPORTED_MODULE_0__.getResource)('qfqf').then(res => createCards(res.styles)).catch(error => console.log(error));
    this.remove();
  });
...
(0,_modules_modules__WEBPACK_IMPORTED_MODULE_0__["default"])();
